document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateCartDisplay() {
        const [productTitleElement, productAmountElement, productPriceElement, productTotalElement] =
            ['product-title', 'product-amount', 'product-price', 'product-total'].map(id => document.getElementById(id));
        let totalPrice = 0;

        [productTitleElement, productAmountElement, productPriceElement, productTotalElement].forEach(el => el.innerHTML = '');
    ...
    document.addEventListener('click', event => {
        if (event.target.classList.contains('remove-btn')) {
            ...
            for (const key in totalAmounts) {
                for (let i = 0; i < totalAmounts[key]; i++) {
                    updatedCartItems.push(key);
                }
            }
